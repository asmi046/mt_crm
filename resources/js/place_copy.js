document.addEventListener("DOMContentLoaded", () => {
    ...
    for (let i = 0; i < all_copy.length; i++)
        all_copy[i].addEventListener("click", function (e) {
            ...
            localStorage.setItem("bufer_nd", document.getElementById("place_nd_"+elements_id).value)
            localStorage.setItem("bufer_dr", document.getElementById("place_dr_"+elements_id).value)
    ...
    for (let i = 0; i < all_paste.length; i++)
        all_paste[i].addEventListener("click", function (e) {
            ...
            document.getElementById("place_nd_"+elements_id).value = localStorage.getItem("bufer_nd")
            document.getElementById("place_dr_"+elements_id).value = localStorage.getItem("bufer_dr")
